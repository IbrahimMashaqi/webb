...
// File: /heels/js/global-functions.js
// Load this AFTER cart.js, auth.js, and wishlist.js
// NO WISHLIST CODE HERE (moved to wishlist.js)
// ============================================
...
window.applyDiscountCode = function (code) {
  ...
  const validCodes = {
    ...
    BESPOKE10: { discount: 0.1, description: "10% off bespoke orders" },
  };
  ...
  if (discountInfo) {
    localStorage.setItem(
      "appliedDiscount",
      JSON.stringify({ code, ...discountInfo })
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".store-btn").forEach((btn) => {
    btn.onclick = function () {
      const storeName = this.closest(".store-card")
        .querySelector("h3")
        .textContent.split(" ")[0];
